...
if [ ! -f "$DB_PATH" ]; then
  cat <<EOF
{"additionalContext": "## ccwire: DB未初期化\n\nccwire DBがまだ作成されていません。\nwire_register(name=\"${CCWIRE_SESSION_NAME}\") を実行してセッションを登録してください。"}
EOF
  exit 0
fi
...
if [ -n "$CCWIRE_TMUX_TARGET" ]; then
  TMUX_TARGET="'$CCWIRE_TMUX_TARGET'"
fi

# 既存の registered_at を保持、なければ現在時刻
EXISTING_REGISTERED=$(sqlite3 "$DB_PATH" "SELECT registered_at FROM sessions WHERE name = '$CCWIRE_SESSION_NAME';" 2>/dev/null)
...
sqlite3 "$DB_PATH" "INSERT OR REPLACE INTO sessions (name, tmux_target, status, registered_at, last_seen) VALUES ('$CCWIRE_SESSION_NAME', $TMUX_TARGET, 'idle', '$REGISTERED_AT', '$NOW');" 2>/dev/null
